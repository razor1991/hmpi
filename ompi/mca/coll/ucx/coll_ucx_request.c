...
void mca_coll_ucx_coll_completion(void *request, ucs_status_t status)
{
    ...
    COLL_UCX_VERBOSE(8, "send request %p completed with status %s", (void*)req, ucs_status_string(status));
...
static void mca_coll_ucx_persistent_op_detach(mca_coll_ucx_persistent_op_t *preq, ompi_request_t *tmp_req)
{
    tmp_req->req_complete_cb_data = NULL;
    preq->tmp_req                 = NULL;
}

inline void mca_coll_ucx_persistent_op_complete(mca_coll_ucx_persistent_op_t *preq, ompi_request_t *tmp_req)
{
...
static void mca_coll_ucx_request_init_common(ompi_request_t* ompi_req, bool op_persistent, ompi_request_state_t state,
                                             ompi_request_free_fn_t req_free, ompi_request_cancel_fn_t req_cancel)
{
...
void mca_coll_ucx_request_init(void *request)
{
    ...
    mca_coll_ucx_request_init_common(ompi_req, false, OMPI_REQUEST_ACTIVE, mca_coll_ucx_request_free,
...
{
    mca_coll_ucx_request_init_common(&req->ompi, true, OMPI_REQUEST_INACTIVE, mca_coll_ucx_persistent_op_free,
...
OBJ_CLASS_INSTANCE(mca_coll_ucx_persistent_op_t, ompi_request_t, mca_coll_ucx_persisternt_op_construct,
...
{
    mca_coll_ucx_request_init_common(ompi_req, false, OMPI_REQUEST_ACTIVE, mca_coll_completed_request_free,
